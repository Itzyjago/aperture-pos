// Aperture Commerce Cloud — deterministic retail seed data.
// A seeded PRNG keeps every figure stable across runs so demos and
// screenshots are reproducible. Numbers are modeled on a mid-size
// multi-store specialty-retail chain (~6 stores, premium apparel & home).
...
// -------------------------------------------------------------- KPI cards
function spark(base, vol, n = 16, up = true) {
  const out = [];
  let v = base;
  for (let i = 0; i < n; i++) {
...
const kpis = [
  {
    ...
    unit: "currency",
    delta: 8.4,
    target: round(netSalesTotal * 1.05),
    spark: spark(netSalesTotal / 16, netSalesTotal / 40),
  ...
  },
  {
    ...
    unit: "number",
    delta: 4.1,
    target: round(txnTotal * 1.08),
    spark: spark(txnTotal / 16, txnTotal / 30),
    hint: "tickets closed",
  },
  {
    key: "avg_basket",
    label: "Avg Basket",
    value: round(netSalesTotal / txnTotal, 2),
    unit: "currency",
    delta: 3.7,
    target: round((netSalesTotal / txnTotal) * 1.04, 2),
    spark: spark(120, 9),
    hint: "value per ticket",
  },
  {
    key: "gross_margin",
    label: "Gross Margin",
  ...
  },
  {
    ...
    delta: 0.9,
    target: 2.5,
    spark: spark(2.3, 0.12),
    hint: "attach & cross-sell",
  },
  {
    key: "sales_sqft",
    label: "Sales / sq-ft",
    value: round(netSalesTotal / stores.reduce((a, s) => a + s.sqft, 0), 2),
    unit: "currency",
    delta: 5.5,
    target: 95,
    spark: spark(78, 5),
    hint: "space productivity",
  },
  {
    key: "shrinkage",
    label: "Shrinkage",
...
// ------------------------------------------------------- Hourly sales (open hours)
const hourlySales = Array.from({ length: 13 }, (_, i) => {
  const hour = i + 9; // 9am → 9pm
...
const topSkus = SKU_NAMES.map(([name, cat], i) => {
...
const products = PRODUCT_DEFS.map(([name, category, brand, price], i) => {
  ...
  const status = stock === 0 ? "out-of-stock" : stock < 25 ? "low-stock" : "active";
  return {
    sku: `SKU-${4100 + i}`,
    name,
    category,
...
// --------------------------------------------------- Region performance
const regionPerf = Object.values(
  stores.reduce((acc, s) => {
